...
function get_user_topic_func($xmlrpc_params)
{
    ...
    $search_fields = 'firstpost';   // only search for topics
    ...
    //          trigger_error('NO_SEARCH_RESULTS');
                return get_error(18, 'No suitable matches were found.');
            }
        }
    ...
    //      trigger_error($error);
        ...
        if ($sql_where)
        {
            ...
            $sql_select = 't.*, f.forum_name, u.user_avatar, u.user_avatar_type, tw.notify_status, bm.topic_id as bookmarked';
    ...
    if(isset($template->_tpldata['searchresults']))
    {
        foreach($template->_tpldata['searchresults'] as $row)
        {
            ...
            $xmlrpc_topic = new xmlrpcval(array(
                ...
                'last_reply_author_name'  => new xmlrpcval(html_entity_decode($row['LAST_POST_AUTHOR']), 'base64'),
                'last_reply_time'   => new xmlrpcval(mobiquo_iso8601_encode($row['LAST_POST_TIME']), 'dateTime.iso8601'),
